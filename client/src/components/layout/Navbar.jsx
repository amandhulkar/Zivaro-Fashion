...
const Navbar = () => {
  ...
  const renderNavLink = (link, extraProps = {}) => <Link key={link.label} to={link.path} className={`${baseLinkClass} ${isNavActive(link.path) ? 'text-accent' : 'text-primary'}`} {...extraProps}>{link.label}</Link>
  const linkClass = ({ isActive }) => `${baseLinkClass} ${isActive ? 'text-accent' : 'text-primary'}`
  const profilePath = isAuthenticated ? '/profile' : '/login'

  const icons = <><button aria-label="Search" onClick={() => setSearchOpen(true)} className="transition hover:text-accent"><FiSearch /></button><NavLink to="/wishlist" aria-label="Wishlist" className="relative transition hover:text-accent"><FiHeart /><CountBadge count={wishlistCount} /></NavLink><NavLink to="/cart" aria-label="Cart" className="relative transition hover:text-accent"><FiShoppingBag /><CountBadge count={cartCount} /></NavLink>{user?.role === 'admin' && <NavLink to="/admin/dashboard" aria-label="Admin Dashboard" className="transition hover:text-accent"><FiSettings /></NavLink>}<NavLink to={profilePath} aria-label="Profile" className="transition hover:text-accent"><FiUser /></NavLink>{isAuthenticated && <button aria-label="Logout" onClick={handleLogout} className="transition hover:text-accent"><FiLogOut /></button>}</>
